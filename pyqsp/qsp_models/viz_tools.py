...
def compute_qsp_response(model=None, phis=None, return_all=False, show_svg=False):
...
def plot_qsp_response(f, model=None, phis=None, title="QSP Response"):
        ...
        """
        qsp_response, all_th, circuit_px, circuit_qx = compute_qsp_response(model=model, phis=phis, return_all=True)

        pdata = {"x": np.cos(all_th), "Imag[p(x)]": np.imag(circuit_px), 
